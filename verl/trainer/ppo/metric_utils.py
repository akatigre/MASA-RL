...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if pos_batch.batch['responses'].shape[0] != 0:
        ...
        pos_metrics = {
            # score
            "critic_pos/score/mean": torch.mean(pos_sequence_score).detach().item(),
            "critic_pos/score/max": torch.max(pos_sequence_score).detach().item(),
            "critic_pos/score/min": torch.min(pos_sequence_score).detach().item(),
            # reward
            "critic_pos/rewards/mean": torch.mean(pos_sequence_reward).detach().item(),
            "critic_pos/rewards/max": torch.max(pos_sequence_reward).detach().item(),
            "critic_pos/rewards/min": torch.min(pos_sequence_reward).detach().item(),
            # adv
    ...
    neg_batch = batch[sequence_score <= 0]
    if neg_batch.batch['responses'].shape[0] != 0:
        ...
        neg_metrics = {
            # score
            'critic_neg/score/mean':
                torch.mean(neg_sequence_reward).detach().item(),
            'critic_neg/score/max':
                torch.max(neg_sequence_reward).detach().item(),
            'critic_neg/score/min':
                torch.min(neg_sequence_reward).detach().item(),
            # reward
            'critic_neg/rewards/mean':
                torch.mean(neg_sequence_reward).detach().item(),
            'critic_neg/rewards/max':
                torch.max(neg_sequence_reward).detach().item(),
            'critic_neg/rewards/min':
                torch.min(neg_sequence_reward).detach().item(),
            # adv
            'critic_neg/advantages/mean':
                torch.mean(neg_valid_adv).detach().item(),
            'critic_neg/advantages/max':
                torch.max(neg_valid_adv).detach().item(),
            'critic_neg/advantages/min':
                torch.min(neg_valid_adv).detach().item(),
            # returns
            'critic_neg/returns/mean':
                torch.mean(neg_valid_returns).detach().item(),
            'critic_neg/returns/max':
                torch.max(neg_valid_returns).detach().item(),
            'critic_neg/returns/min':
                torch.min(neg_valid_returns).detach().item(),

            # response length
            'response_length/neg/mean':
                torch.mean(neg_response_length).detach().item(),
            'response_length/neg/max':
                torch.max(neg_response_length).detach().item(),
            'response_length/neg/min':
                torch.min(neg_response_length).detach().item(),
            'response_length/neg/clip_ratio':
                torch.mean(torch.eq(neg_response_length, neg_max_response_length).float()).detach().item(),
            # prompt length
            'prompt_length/neg/mean':
                torch.mean(neg_prompt_length).detach().item(),
            'prompt_length/neg/max':
                torch.max(neg_prompt_length).detach().item(),
            'prompt_length/neg/min':
                torch.min(neg_prompt_length).detach().item(),
            'prompt_length/neg/clip_ratio':
                torch.mean(torch.eq(neg_prompt_length, neg_max_prompt_length).float()).detach().item(),
        }
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        [(bon_mean, bon_std), (won_mean, won_std)] = bootstrap_metric(
                            data=var_vals, subset_size=n, reduce_fns=[
                                np.max, np.min], seed=seed
                        )
        ...
        for prompt, var2metric in prompt2var2metric.items():
            for var_name, metric in var2metric.items():
                for metric_name, metric_val in metric.items():
                    data_src2var2metric2prompt_vals[data_source][var_name][metric_name].append(
                        metric_val)
