...
class ResourcePoolManager:
    ...
    def _check_resource_available(self):
        ...
        node_available_gpus = {
            node: node_info.get(
                "GPU", 0) if "GPU" in node_info else node_info.get("NPU", 0)
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns, id2mean = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.meta_feedin = config.data.get('meta_feedin', False)
        self.meta_sft = config.get('meta_sft', None)
    ...
    def _validate_config(self):
        ...
        if config.actor_rollout_ref.actor.strategy == "megatron":
            ...
            assert (
                n_gpus % (model_parallel_size *
                          config.actor_rollout_ref.actor.megatron.context_parallel_size) == 0
            ), (
    ...
    def _save_checkpoint(self):
        ...
        max_actor_ckpt_to_keep = (
            self.config.trainer.get(
                "max_actor_ckpt_to_keep", None) if not remove_previous_ckpt_in_save else 1
        )
        max_critic_ckpt_to_keep = (
            self.config.trainer.get(
                "max_critic_ckpt_to_keep", None) if not remove_previous_ckpt_in_save else 1
        )
    ...
    def _load_checkpoint(self):
        ...
        if self.config.trainer.resume_mode == "auto":
            ...
            if self.config.trainer.resume_mode == "resume_path":
                assert isinstance(
                    self.config.trainer.resume_from_path, str), "resume ckpt must be str type"
