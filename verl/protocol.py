...
class _DataProtoConfigMeta(type):
    ...
    def auto_padding(cls, enabled: bool):
        assert isinstance(
            enabled, bool), f"enabled must be a boolean, got {enabled} as {type(enabled)}"
...
class DataProto:
    ...
    def check_consistency(self):
        ...
        if self.batch is not None and self.non_tensor_batch is not None and len(self.non_tensor_batch) != 0:
            ...
            assert len(
                self.batch.batch_size) == 1, "only support num_batch_dims=1 when non_tensor_batch is not empty."
    ...
    def from_dict(
        ...
        tensor_dict = TensorDict(
            source=tensors, batch_size=batch_size) if tensors else None
        if auto_padding:
    ...
    def select(self, batch_keys=None, non_tensor_batch_keys=None, meta_info_keys=None, deepcopy=False) -> "DataProto":
        ...
        if non_tensor_batch_keys is not None:
            non_tensor_batch = {key: val for key, val in self.non_tensor_batch.items(
    ...
    def make_iterator(self, mini_batch_size, epochs, seed=None, dataloader_kwargs=None):
        ...
        """
        assert self.batch.batch_size[
            0] % mini_batch_size == 0, f"{self.batch.batch_size[0]} % {mini_batch_size} != 0"
    ...
    def chunk(self, chunks: int) -> list["DataProto"]:
        ...
        for i in range(chunks):
            output.append(
                type(self)(
                    batch=batch_lst[i], non_tensor_batch=non_tensor_batch_lst[i], meta_info=self.meta_info)
            )
    ...
    def reorder(self, indices):
        ...
        self.non_tensor_batch = {key: val[indices_np]
                                 for key, val in self.non_tensor_batch.items()}
    ...
    def repeat(self, repeat_times=2, interleave=True):
        ...
        if self.batch is not None:
            if interleave:
                ...
                # Stack the data
                repeated_tensors = {
                    key: tensor.unsqueeze(0).expand(
                        repeat_times, *tensor.shape).reshape(-1, *tensor.shape[1:])
                    for key, tensor in self.batch.items()
                }
...
def all_gather_data_proto(data: DataProto, process_group):
    ...
    data.non_tensor_batch = {k: np.concatenate(
        [d[k] for d in all_non_tensor_batch]) for k in data.non_tensor_batch}
