...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def meta_sft_fit(self, prompts, responses, meta_sft_config, logger):
        assert self._is_actor, "meta SFT training requires actor role"
        from verl.trainer.meta_sft_dataset import SFTDataset
        from verl.trainer.fsdp_sft_trainer import FSDPSFTTrainer
        meta_sft_lr = self.config.meta_sft.optim.lr

        dataset = SFTDataset(prompts, responses, self.tokenizer, meta_sft_config.data)
        original_lr = self.actor_optimizer.param_groups[0]['lr']
        for group in self.actor_optimizer.param_groups:
            group['lr'] = meta_sft_lr
        trainer = FSDPSFTTrainer(
            config=meta_sft_config,
            device_mesh=self.device_mesh,
            sharding_manager=self.ulysses_sharding_manager,
            ulysses_device_mesh=self.ulysses_device_mesh,
            tokenizer=self.tokenizer,
